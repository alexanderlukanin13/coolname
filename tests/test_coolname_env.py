...
def generate_slugs(number_of_slugs: int, data_dir: Path | None = None, data_module=None, path: Path | None = None, expect_returncode=0):
    env = dict(os.environ)
    env['PYTHONPATH'] = str(PROJECT_DIR)
    if path:
        env['PYTHONPATH'] += os.pathsep + str(PROJECT_DIR / path)
    if data_dir:
        env['COOLNAME_DATA_DIR'] = str(data_dir)
    if data_module:
...
def test_coolname_env():
    ...
    assert generate_slugs(len(RUSSIAN), data_dir=EXAMPLES_DIR / 'russian') == RUSSIAN
    ...
    assert generate_slugs(len(RUSSIAN_M),
                          data_dir=EXAMPLES_DIR / 'no_such',
                          data_module='russian_module',
                          path=EXAMPLES_DIR) == RUSSIAN_M
    # Only COOLNAME_DATA_DIR, and it is invalid
    lines = generate_slugs(1, data_dir=Path('no_such'), expect_returncode=1)
